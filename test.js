...
class App {
    constructor() {
        ...
        this._minimapContainer = document.getElementById('minimap');
        this._setupMinimap();
    ...
    _setupMinimap() {
        ...
        // 미니맵 이미지 설정
        minimapContainer.style.backgroundImage = "url('./data/campus.png')"; // campus.png 파일의 경로를 업데이트하세요
        minimapContainer.style.backgroundSize = 'cover';
        minimapContainer.style.backgroundPosition = 'center';
        ...
        minimapContainer.addEventListener('click', (event) => {
            ...
            // 클릭 좌표를 이용한 추가 로직 처리
            this._moveMainCameraToMinimapPosition(x, y);

            this._updateMinimap(x, y);
        });
    }

    _moveMainCameraToMinimapPosition(minimapX, minimapY) {
        console.log('Move Main Camera to Minimap Position');
        const worldSize = 3000;
    
        // 클릭한 미니맵 좌표를 월드 좌표로 변환
        const worldX = (2 * minimapX / this._minimapContainer.clientWidth) * worldSize - 1000;
        const worldY = (-2 * minimapY / this._minimapContainer.clientHeight) * worldSize + 4500;

        console.log('New Camera Position:', worldX, worldY, 0);
    
        const cameraHeight = 3000;
        const newCameraPosition = new THREE.Vector3(worldX, worldY, cameraHeight);
        const newCameraTarget = new THREE.Vector3(worldX, worldY, 0);
    
        // 이동할 위치로 메인 카메라 시점 변경
        this._camera.position.copy(newCameraPosition);
        this._camera.lookAt(newCameraTarget); // 모델을 바라보도록 설정
    
        this._controls.target.set(worldX, worldY, 0);
    
        this.update();
    }

    _updateMinimap(minimapX, minimapY) {
        console.log('updateMinimap');

        const existingDot = document.querySelector('.minimap-dot');
        if (existingDot) {
            existingDot.remove();
        }
        ...
        redDot.className = 'minimap-dot';
        redDot.style.position = 'absolute';
        redDot.style.top = `${minimapY}px`;
        redDot.style.left = `${minimapX}px`;
        ...
        // 미니맵 컨테이너에 빨간 점 추가
        const minimapContainer = document.getElementById('minimap');
        minimapContainer.appendChild(redDot);
        console.log('draw red dot');
    }
    ...
    _setupModel() {
        ...
        this._camera.position.set(0, 0, 3000);
        this._camera.lookAt(model.position);

        this._camera.lookAt(this._model.position);
    });
